...
namespace CleverGirl
{
    public class CandidateWeapons
    {
        ...
        public CandidateWeapons(AbstractActor attacker, ICombatant target)
        {
            ...
            Mod.Log.Debug?.Write($"Condensing candidate weapons for attacker: {attacker.DistinctId()}");
            for (int i = 0; i < attacker.Weapons.Count; i++)
            {
                ...
                if (weapon.CanFire)
                {
                    Mod.Log.Debug?.Write($" -- weapon '{weapon.defId}' canFire, condensing");
                ...
                {
                    Mod.Log.Debug?.Write($" -- weapon '{weapon.defId}' excluded; disabled or out of ammo");
                }
            }
            Mod.Log.Debug?.Write($" -- DONE condensing to {condensed?.Count} distinct weapons");
            ...
            Mod.Log.Debug?.Write($" Validating range {distance} and LOF from attacker: {attacker.CurrentPosition} to target: {target.CurrentPosition}");
            foreach (KeyValuePair<string, CondensedWeapon> kvp in condensed)
            {
                ...
                Mod.Log.Debug?.Write($" --- base weaponAndAmmo has willFireAtTarget: {willFireAtTarget}  withinRange: {withinRange}");
                ...
                if (firingMethods != null && firingMethods.Count > 0)
                {
                    foreach (AmmoModePair item in rawWeapon.getAvaibleFiringMethods())
                    {
                        ...
                        Mod.Log.Debug?.Write($" ---- ammoMode: {item.ammoId}_{item.modeId} for weapon: {rawWeapon.UIName} has modeWillFire: {modeWillFire} modeInRange: {modeInRange}");

                        if (modeWillFire && modeInRange) 
                        { 
                            cWeapon.ammoModes.Add(item); 
